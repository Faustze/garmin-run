...
uv run garmin-run dashboard --open    # дашборд недели в браузере (data/dashboard.html)
...
`dashboard` собирает из `data/`, `plan/` и `journal/` одну HTML-страницу: дни до старта, неделя план/факт
по дням, сигналы усталости по правилам тренера, доля лёгких в пульсе, объём по неделям (факт против плана),
HRV, пульс покоя, сон и Body Battery за 28 дней, текущая фаза макроплана, правки недели и последний разбор.
Без сети и библиотек — встроенный SVG. Файл лежит в `data/`, то есть в git не попадает: там здоровье.
Идея — из [claude-coach](https://github.com/dwedigital/claude-coach): терминал остаётся главным, дашборд — очки поверх.

## Воскресный разбор

Каждое воскресенье в 19:00 планировщик Windows открывает окно и запускает `scripts/weekly-brief.sh`:
забирает данные Garmin, показывает сводку и открывает дашборд, дальше — `claude` в этой папке и «тренер».
...
Раз в неделю в календаре стоит «Силовая ноги» — описание и техника каждого упражнения со ссылкой на видео
в шагах тренировки (Garmin Connect → Календарь → тренировка). На часах её записывать не нужно.
Сделал — отметь в Garmin Connect: «+» → «Добавить активность вручную» → «Силовая», длительность.
После `pull` дашборд и тренер засчитают день.
